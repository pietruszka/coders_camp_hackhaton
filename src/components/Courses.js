...
class Courses extends PureComponent {
    ...
    notification.open({
      message: (
        <div style={{ color: '#60712f', fontSize: '18px' }}>
          Dobrze!! Dostajesz {this.monster.exp + Math.floor(this.props.profile.Intelligence / 10)}{' '}
          Doświadczenia <br /> Zadałeś {this.props.profile.Strength} obrażeń
        </div>
      ),
      description: (
  ...
  openNotificationerror = () => {
    notification.open({
      message: (
        <div style={{ color: '#60712f', fontSize: '18px' }}>
          Zła odpowiedź, tracisz {this.monster.damage} HP !!
        </div>
      ),
  ...
  openNotificationdeath = () => {
    notification.open({
      message: (
        <div style={{ color: '#60712f', fontSize: '18px' }}>
          Umarłeś!! Tracisz {Math.floor(this.props.profile.EXP / 2)} Doświadczenia{' '}
        </div>
      ),
      // description: (<div style={{ display: 'flex', justifyContent: 'center' }}><img src={helmet} /></div>),
    });
  ...
  openNotificationdodge = () => {
    notification.open({
      message: (
        <div style={{ color: '#60712f', fontSize: '18px' }}>Zła odpowiedź ale unikasz obrażeń!</div>
      ),
      // description: (<div style={{ display: 'flex', justifyContent: 'center' }}><img src={helmet} /></div>),
    });
  };
  ...
  onAnswClick = isCorrect => {
    if (isCorrect) {
      ...
      this.setState({ question: this.getRandomQuestion() });
    } else if (Math.random() + this.props.profile.Agility / 20 > 0.9) {
